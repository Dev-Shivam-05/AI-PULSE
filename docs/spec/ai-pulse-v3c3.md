...
Status: built 2026-08-23 on `v3-phase-c`.
...
| 6 | A thumbnail headline is measured against the frame it is drawn on | `_fit_font(draw, lines, start_size, budget_px)` (new, pure) shrinks by the same step 6 / floor 72 that `make_tool_thumb` already uses, applied in `compose` and `_text_block` | Both composers picked a size from a fixed 3-step ladder keyed on **character count** and drew it at x=54/56 with no measurement. `OPENAI QUIETLY SHIPPED A NEW REASONING MODEL` measures 1,296px at the ladder's own floor of 92px and runs 72px off a 1,280px frame. `make_tool_thumb` — written later, for v3 — already does this correctly; the two older composers never got it |
...
## OUT OF SCOPE
- The 15 REFUTED candidates below.
...
- [ ] A stat card is rendered at the duration `step5_build` will give it, for both a
      long scene (no loop) and a short one (no truncated count-up)
- [ ] `_count_seq(stat, 1.0) == stat` for every stat the regex can match
- [ ] No two `build_ass` Dialogue events overlap, for word timings that abut exactly
- [ ] Re-running the clamp over all 24 archived caption files leaves 0 overlaps
- [ ] A 6-word hook wraps to 2 lines that both measure inside 1080px, with no word dropped
- [ ] `normalize_moments` survives a string `scene_num`, a null `hook_text`, a bare-string
      entry and a dict-shaped `moments`, and `make_shorts` returns `[]` instead of raising
- [ ] A 44-character headline is drawn inside the frame in `compose` and `_text_block`
- [ ] An empty or whitespace `thumb_text` still produces a thumbnail with text on it
- [ ] `l2.splice` returns `None` on failure and the clip stays unused and unrecorded
- [ ] `state/l2_usage.json` and `state/stock_ledger.json` are in both the CI stash list
      and `state_merge.FILES`
- [ ] A scene whose second clip fails to encode still fills its full duration
- [ ] The full suite is green, and the artifacts were produced and inspected
