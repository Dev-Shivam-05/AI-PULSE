...
| **v3-B.1: tool-lane diagnosis** | the pivot has never run unattended: **0 of 33** videos published 2026-08-24 → 2026-09-25 were `format=tool`, although `"tool_format": true` (21 news, 11 evergreen, 1 roundup). Read the CI logs for `🧰` / `⛔ Skipping tool candidate` / `↻` / `No tool script — falling back` and find which gate or feed stops it | ⏳ next | needs the owner to open the Actions logs (no `gh` here) — or a supervised `format=tool` dispatch |
...
2. **v3-B.1 — why the tool lane never publishes** (0 of 33 since 2026-08-24). This outranks
   any learning-loop v2: the loop can only tune the lanes that actually run. v3-D's scoreboard
   prints in every CI analytics step; v2 (format choice, packaging) waits until arms are
   trusted (≥5 mature videos AND ≥100 views) — at today's ~4 views per long-form that is weeks.
